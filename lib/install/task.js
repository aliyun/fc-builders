...
class Task {
  constructor(name, { cwd, env = {}, target, registryUrl } = {}) {
    this.name = name;
    this.cwd = cwd;
    this.env = env;
    this.target = target;
  ...
  resolveCmd(cmd, registryUrl, registryParam) {
    if (!registryUrl) {
      return cmd;
    }
    return _.concat(cmd, registryParam, registryUrl);
  }
}
...
class PipTask extends InstallTask {
  ...
  async doRun() {
    ...
    const cmd = this.resolveCmd(['pip', 'install', '--user', '--upgrade', this.pkgName], this.registryUrl, '-i');
...
class NpmTask extends InstallTask {
  ...
  async doRun() {
    ...
    const cmd = this.resolveCmd(['npm', 'install', '-q', '--no-audit', '--production', this.pkgName], this.registryUrl, '--registry');
