    ...
    console.log(green('     => ') + cyan(`bash -c
        for f in $(ls %s/archives/*.deb); do
          ...
          if [ -f "%s/deb-control/\${f%.*}/postinst" ]; then
            ${prefix}FUN_INSTALL_LOCAL=true DPKG_MAINTSCRIPT_NAME=postinst sudo %s/deb-control/\${f%.*}/postinst configure || true;
          fi;
    ...
    await this._exec(['bash', '-c', '-O', 'nullglob',
      `for f in ${this.cacheDir}/archives/*.deb; do
          ...
          if [ -f "${this.cacheDir}/deb-control/\${f%.*}/postinst" ]; then
            ${prefix}FUN_INSTALL_LOCAL=true DPKG_MAINTSCRIPT_NAME=postinst sudo ${this.cacheDir}/deb-control/\${f%.*}/postinst configure || true; 
          fi;
      done;`
    ]);
  }
