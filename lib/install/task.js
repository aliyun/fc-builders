...
class PipTask extends InstallTask {
  constructor(cwd, pkgName, env, target) {
    super("PipTask", cwd, pkgName, env, target);
  }
...
class NpmTask extends InstallTask {
  constructor(cwd, pkgName, env, target) {
    super("NpmTask", cwd, pkgName, env, target);
  }

  async doRun() {
    await super.doRun();
    
    const cmd = ['npm', 'install', '-q', '--no-audit', '--production', this.pkgName];
...
class AptTask extends InstallTask {
  ...
  constructor(cwd, pkgName, env, target) {
    super("AptTask", cwd, pkgName, env, target);
    ...
    await this._exec(['bash', '-c', '-O', 'nullglob',
      `for f in ${this.cacheDir}/archives/*.deb; do
          ...
          if [ -f "${this.cacheDir}/deb-control/\${f%.*}/postinst" ]; then 
            ${prefix}FUN_INSTALL_LOCAL=true DPKG_MAINTSCRIPT_NAME=postinst ${this.cacheDir}/deb-control/\${f%.*}/postinst configure; 
          fi; 
      done;`
    ]);
  }
