...
class PipTask extends InstallTask {
  ...
  async doRun() {
    ...
    log.info(green('     => ') + cyan(`PYTHONUSERBASE=${pythonUserBase} pip install --user --upgrade %s`), this.pkgName);

    await this._exec(['pip', 'install', '--user', '--upgrade', this.pkgName], this.cwd,
      Object.assign({
