...
class Builder {
  constructor(serviceName, functionName, sourceDir, runtime, artifactDir, verbose) {
  ...
  async build() {
    log.debug('builder begin to build, runtime is: ' + this.runtime + ' sourceDir: ' + this.sourceDir);

    const taskFlows = await Builder.detectTaskFlow(this.runtime, this.sourceDir);

    if (!taskFlows) {
      throw new Error('could not found TaskFlow for ' + this.runtime);
    }
