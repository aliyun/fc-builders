...
CHECK_FILE=oss://oss-attachment/fun-install/fun-install-v${VERSION}-linux-x64.zip
...
do
    echo $p
    ossutil cp $p oss://oss-attachment/fun-install/$p --endpoint $ENDPOINT --access-key-id $OSS_ACCESS_KEY_ID --access-key-secret $OSS_ACCESS_KEY_SECRET
    files+=($p)
done
...
for f in "${files[@]}"
do
   echo "[$f](https://gosspublic.alicdn.com/fun-install/$f)"
done
